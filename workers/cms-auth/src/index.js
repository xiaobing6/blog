...
const handleCallback = async (request, env) => {
	...
	if (provider === 'github' && token) {
		const allowedUsers = (ALLOWED_GITHUB_USERS ?? '')
			.split(',')
		...
		if (allowedUsers.length === 0) {
			return outputHTML({
				provider,
				error: 'The GitHub account allowlist is not configured.',
				errorCode: 'MISCONFIGURED_USER_ALLOWLIST',
			});
		}
		...
		const userURL =
			GITHUB_HOSTNAME === 'github.com'
				...
				: `https://${GITHUB_HOSTNAME}/api/v3/user`;
		let userResponse;
		let user;
		...
		try {
			userResponse = await fetch(userURL, {
				headers: {
					...
					'User-Agent': 'xiaobing6-blog-cms-auth',
		...
		if (!userResponse?.ok || !user?.login) {
			return outputHTML({
				provider,
				error: 'Failed to verify the authorized GitHub account.',
		...
		if (!allowedUsers.includes(user.login.toLowerCase())) {
			return outputHTML({
				provider,
				error: 'This GitHub account is not allowed to access the CMS.',
